...
\hline
\hline
Components              & Key Factors (Speed, Image Quality, {and} User Experience)       \\
\hline
...
{To} achieve these objectives, {all parts} of the system should be considered, as shown in Table~\ref{table_factors}.
...
The resolution of this CIS sensor is \SI{100}{dpi}, and can be configured to \SI{200}{dpi} and \SI{50}{dpi}. The resolution of the sensor determines the horizontal resolution of the captured image. The optimal resolution is \SI{100}{dpi} according to {studies} of the physical resolution of {palmprint features by Shu and Zhang\cite{Shu1998}, Zhang et al.\cite{Zhang1999}, Zhang and Shu\cite{Zhang2003a}, and Wong et al.\cite{Wong2005}.}

This CIS sensor scans at a speed of \SI{45}{\mu s} per line in \SI{100}{dpi} mode. It captures 720 lines at a time to get the full hand image including the wrist part. {Image capturing} takes \SI{33}{ms}. This capturing speed ensures that the processing time {is} not delayed by the sensor. However, the entire processing time depends on the movement of hands.
...
\caption{The controller board of {LPS}. (a) Scheme block diagram. (b) {Printed circuit board layout.}}
...
In our prototype device, the photoelectric encoder is industry standard {at} 500 pulses per round ($P_e=500$). The rollers' diameter is \SI{10}{mm}. The gear ratio $R_g$ is $2 : 1$. The filter ratio $R_f$ is $2 : 1$. Under this condition, the vertical resolution (along the rolling direction) is \SI{101.1}{dpi}, which is close to the resolution along the width direction of the CIS module (\SI{100}{dpi}).

\subsection{Controller Board}

An FPGA board was built as the controller. It {is composed of} a CIS driver, an A/D controller, {a data buffer, and a USB interface.} The block diagram of the controller is shown in Fig.~\ref{fig_block_controller}.
...
\caption{The proposed acquisition system. (a) The 3D design model of the acquisition system. (b) A right hand was {being tested} on the acquisition system.}
\label{fig_demo}
\end{figure*}
...
Cypress USB CY7C68013A is used as the USB interface engine. It works in Slave FIFO mode for the highest transferring speed and {with} minimum latency. Continuous transmission speed is over \SI{18}{MiB/s} in dummy data test. {Both the PC-based and embedded platform-based drivers are implemented.}

{The printed circuit board layout} is shown in Fig.~\ref{fig_layout_controller}. The width of this PCB is the same as the width of the synchronizing unit. In this way, the width is minimized to the width of the synchronizing unit. Both the CIS module and the photoelectric encoder are powered by this board. There is only one {4-pin} \SI{12}{V} to \SI{5}{V} power module being used here as the power supply for the entire device.
...
To compare with current real-time online area based palmprint systems\cite{Zhang2003a, Wong2005, Zhang2010}, we {needed} the recognition performance of a verification experiment of the proposed system working in the same environment.

Fig.~\ref{fig_expflow} {shows} the flowchart of the experiment. The experiment was designed following the area palmprint systems' convention. This experiment consisted of four parts: image capturing, ROI extraction, feature extraction, and matching. In image capturing, the proposed {LPS} has been used.
...
\caption{Five-step ROI extraction. (a) {a palmprint sample; (b) binarization result; (c) contour extraction; (d) tangents points found; (e) located ROI; and (f) extracted ROI.}}
...
The EER of the verification experiment is 0.048\% when {the FAR equals the FRR.} Compared with area palmprint systems, this result is better than {the results of} flatbed scanner and WCBS (from 0.26\% to over 4.73\% (Ong2003, Han2003, Kumar2003, Lin2005, Connie2005, Goh2006, Badrinath2007, [Han et al.][Han2007b], Struc2008, Ong2010)), and lies within the range of the pegged platen palmprint systems (from 0.267\% to 0.012\%\cite{Ito2006, Hao2008, Wang2008, Zuo2008compcode, Guo2009bocv, Laadjel2009a, Guo2009coc, Zhang2010, Zhang2010b, Li2012a}). Although the result is not better than the best result in all area counterparts, the proposed system achieves a verification performance comparable with the best area sensor-based systems.

\subsection{Comparisons with Current Area Sensor-Based Palmprint Systems}

\begin{table*}[!ht]
\renewcommand{\arraystretch}{1.3}
\caption{Comparison between the {LPS} and Area Scan Systems}
\label{table_compare}
\centering
\begin{tabular}{rllll}
\hline
...
A detailed comparison with area sensor-based palmprint system is shown in Table~\ref{table_compare}. The shape of {LPS} is \SI{22}{cm}$\times$\SI{5}{cm}$\times$\SI{5}{cm}. Compared with {current area image sensor-based palmprint acquisition systems} (\SI{32}{cm}$\times$\SI{16}{cm}$\times$\SI{19}{cm}\cite{Zhang2003a}, \SI{32}{cm}$\times$\SI{16}{cm}$\times$\SI{19}{cm}\cite{Wong2005}, \SI{34}{cm}$\times$\SI{28}{cm}$\times$\SI{26}{cm}\cite{Zhang2010}), the proposed system is much smaller. The volume of the proposed system is less than 6\% of the volume of {the area-based ones.} The volume of {area sensor-based systems} could not be reduced further using the same imaging structure. Theoretically, the area-based design could not reduce the shape smaller than \SI{16}{cm}$\times$\SI{16}{cm}$\times$\SI{20}{cm}, considering the space taken by the optical path and the back cover. In contrast, the shape of {LPS} could be further reduced after moderate improvements.
...
Fulfilling the size and speed requirements, {LPS} does not compromise verification performance. {LPS} achieves a verification performance (0.048\%) comparable with most advanced area sensor-based counterparts (0.6\%\cite{Zhang2003a}, 0.021\% to 0.052\%\cite{Zhang2010}) in terms of EER on a databases of a similar scale (8000 images). {LPS} does not reduce the volume by sacrificing recognition performance. Meanwhile, it further shows that {palmprint recognition} can achieve a similar verification performance (EER \textless{} 0.06\%) in different acquisition systems.
