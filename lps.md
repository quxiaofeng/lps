...
A flatbed scanner captures contact palmprint images in high resolution. It has been utilized as a palmprint scanner by many researchers [Ong2003, Han2003, Connie2005, Lin2005, Goh2006, Badrinath2007, Zheng2007a, Struc2008. The scanning time of a flatbed scanner is typically 10s -- 20s for a 300dpi A4 size scan.
...
\captionCIS module and the synchronizing unit composed of a pair of rollers, a set of gears and an optical encoder.
\labelfig_syncunit
\endfigure
...
\centering
\subfloat[]
  \includegraphics[width =4 in]6a\labelfig_block_controller~
\subfloat[]
  \includegraphics[width =1.6 in]6b\labelfig_layout_controller
\captionThe controller board of LPS. (a) Scheme block diagram. (b) Printed circuit board layout.
\labelfig_controller
\endfigure*
...
\centering
\subfloat[]
  \includegraphics[width =4 in]7a\labelfig_device_photo~
\subfloat[]
  \includegraphics[width =1.6 in]7b\labelfig_demo_photo
\captionThe proposed acquisition system. (a) The 3D design model of the acquisition system. (b) A right hand was being tested on the acquisition system.
\labelfig_demo
\endfigure*
...
\captionThe captured palmprint images.
\labelfig_raw
\endfigure*
...
\captionLPS verification experiment flowchart.
\labelfig_expflow
\endfigure
...
\centering
\subfloat[]
  \includegraphics[width=1.5in,natwidth=720,natheight=668]10a\labelfig_rawpalm~
\subfloat[]
  \includegraphics[width=1.5in]10b\labelfig_binarization~
\subfloat[]
  \includegraphics[width=1.5in]10c\labelfig_contour
\hfil
\subfloat[]
  \includegraphics[width=1.5in]10d\labelfig_tangents~
\subfloat[]
  \includegraphics[width=1.5in]10e\labelfig_roiregion~
\subfloat[]
  \includegraphics[width=1.4in,natwidth=460,natheight=461]10f\labelfig_roiexample
\captionFive-step ROI extraction. (a) a palmprint sample; (b) binarization result; (c) contour extraction; (d) tangents points found; (e) located ROI; and (f) extracted ROI.
\labelfig_roiextraction
\endfigure*
...
\captionExtracted palmprint ROIs.
\labelfig_rois
\endfigure*
...
During matching, the Hamming distance is adopted following the best practice of area-based conventionZhang2010, which has proven to be efficient and effective in palmprint feature matching.
...
The EER of the verification experiment is 0.048% when the FAR equals the FRR. Compared with area palmprint systems, this result is better than the results of flatbed scanner and WCBS (from 0.26% to over 4.73% (Ong2003, Han2003, Kumar2003, Lin2005, Connie2005, Goh2006, Badrinath2007, [Han et al.][Han2007b], Struc2008, Ong2010)), and lies within the range of the pegged platen palmprint systems (from 0.267% to 0.012%Ito2006, Hao2008, Wang2008, Zuo2008compcode, Guo2009bocv, Laadjel2009a, Guo2009coc, Zhang2010, Zhang2010b, Li2012a). Although the result is not better than the best result in all area counterparts, the proposed system achieves a verification performance comparable with the best area sensor-based systems.

### Comparisons with Current Area Sensor-Based Palmprint Systems

A detailed comparison with area sensor-based palmprint system is shown in Tabletable_compare. The shape of LPS is 22cm$\times$5cm$\times$5cm. Compared with current area image sensor-based palmprint acquisition systems (32cm$\times$16cm$\times$19cmZhang2003a, 32cm$\times$16cm$\times$19cmWong2005, 34cm$\times$28cm$\times$26cmZhang2010), the proposed system is much smaller. The volume of the proposed system is less than 6% of the volume of the area-based ones. The volume of area sensor-based systems could not be reduced further using the same imaging structure. Theoretically, the area-based design could not reduce the shape smaller than 16cm$\times$16cm$\times$20cm, considering the space taken by the optical path and the back cover. In contrast, the shape of LPS could be further reduced after moderate improvements.
...
Fulfilling the size and speed requirements, LPS does not compromise verification performance. LPS achieves a verification performance (0.048%) comparable with most advanced area sensor-based counterparts (0.6%Zhang2003a, 0.021% to 0.052%Zhang2010) in terms of EER on a databases of a similar scale (8000 images). LPS does not reduce the volume by sacrificing recognition performance. Meanwhile, it further shows that palmprint recognition can achieve a similar verification performance (EER \textless 0.06%) in different acquisition systems.
